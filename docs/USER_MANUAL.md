...
1. 打开设置面板
2. 在"快捷键"区域点击编辑按钮
3. 按下新的快捷键组合（必须包含Ctrl/Shift/Alt修饰键）
4. 系统自动检测冲突（浏览器快捷键或已使用的快捷键）
5. 点击确认保存
6. 点击"重置全部"可恢复默认配置
...
- **即将发货**: 未来几天的发货计划，显示预计发货日期和提前备货天数，每个订单显示满足率进度条
...
点击工具栏的"新建订单"按钮（琥珀色），打开订单编辑弹窗：
- 填写订单基本信息（日期、客户、款号、吨数等）
- 产线支持多选（勾选多个产线）
- 保存后订单添加到系统
...
> **多产线订单说明**: 多产线订单的库存满足率会合并计算所有指定产线的库存，库存明细弹窗中会显示各产线的库存来源
...
| 港口 | 目的港 |
| 状态 | 订单状态（下拉可切换） |
| 满足率 | 库存满足订单的百分比（进度条+数值），点击可查看库存明细和仓库分配 |
| 操作 | 打印、编辑、删除、展开详情 |
...
库存管理标签页顶部显示可视化图表：
- **统计卡片**: 总库存、一般贸易库、保税库、可用库存
- **款号库存柱状图**: 显示TOP8款号的库存分布
- **低库存预警**: 红色标识低于安全库存的款号数量
...
> **备份内容(v2.0)**: 订单、库存（含产线信息）、产线、款号、异常记录、客户、库存流水、审计日志、款号变更历史

> **兼容性**: 恢复功能兼容v1.0和v2.0格式的备份文件

### 快捷键设置

- **查看快捷键**: 显示所有快捷键及其功能
- **编辑快捷键**: 点击编辑按钮，按下新的快捷键组合
- **冲突检测**: 自动检测与浏览器快捷键或其他功能的冲突
- **重置全部**: 恢复所有快捷键为默认值
