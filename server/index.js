...
app.get('/api/inventory', asyncHandler((req, res) => {
  const rows = query('SELECT style_no as styleNo, current_stock as currentStock, grade_a as gradeA, grade_b as gradeB, stock_t_minus_1 as stockTMinus1, locked_for_today as lockedForToday FROM inventory');
  res.json(rows.map(r => ({ ...r, gradeA: r.gradeA || 0, gradeB: r.gradeB || 0 })));
}));

app.put('/api/inventory/:styleNo', asyncHandler((req, res) => {
  const { currentStock, gradeA, gradeB, stockTMinus1, lockedForToday } = req.body;
  const total = (gradeA || 0) + (gradeB || 0);
  run('UPDATE inventory SET current_stock = ?, grade_a = ?, grade_b = ?, stock_t_minus_1 = ?, locked_for_today = ? WHERE style_no = ?', [currentStock || total, gradeA || 0, gradeB || 0, stockTMinus1, lockedForToday, req.params.styleNo]);
  res.json({ success: true });
}));

// 入库（事务处理）
app.post('/api/inventory/in', asyncHandler((req, res) => {
  const { styleNo, quantity, grade, source, note } = req.body;
  if (!styleNo || !quantity) return res.status(400).json({ error: '款号和数量必填' });
  const g = grade || 'A';
  const existing = queryWithParams('SELECT current_stock as currentStock, grade_a as gradeA, grade_b as gradeB FROM inventory WHERE style_no = ?', [styleNo])[0];
  ...
  withTransaction(() => { // 事务包装：确保库存更新和流水记录同时成功或失败
    if (existing) {
      runNoSave('UPDATE inventory SET current_stock = ?, grade_a = ?, grade_b = ? WHERE style_no = ?', [newBalance, newGradeA, newGradeB, styleNo]);
    } else {
      runNoSave('INSERT INTO inventory (style_no, current_stock, grade_a, grade_b) VALUES (?, ?, ?, ?)', [styleNo, newBalance, newGradeA, newGradeB]);
    }
    runNoSave('INSERT INTO inventory_transactions (style_no, type, grade, quantity, balance, source, note) VALUES (?, ?, ?, ?, ?, ?, ?)', [styleNo, 'IN', g, quantity, newBalance, source || null, note || null]);
  });
...
app.post('/api/inventory/out', asyncHandler((req, res) => {
  ...
  withTransaction(() => { // 事务包装：确保库存更新和流水记录同时成功或失败
    runNoSave('UPDATE inventory SET current_stock = ?, grade_a = ?, grade_b = ? WHERE style_no = ?', [newBalance, newGradeA, newGradeB, styleNo]);
    runNoSave('INSERT INTO inventory_transactions (style_no, type, grade, quantity, balance, source, note) VALUES (?, ?, ?, ?, ?, ?, ?)', [styleNo, 'OUT', g, quantity, newBalance, source || null, note || null]);
  });
  res.json({ success: true, balance: newBalance, gradeA: newGradeA, gradeB: newGradeB });
}));
...
app.get('/api/inventory/transactions', asyncHandler((req, res) => {
  ...
  const sql = styleNo
    ? 'SELECT id, style_no as styleNo, type, grade, quantity, balance, source, note, created_at as createdAt FROM inventory_transactions WHERE style_no = ? ORDER BY created_at DESC LIMIT 100'
    : 'SELECT id, style_no as styleNo, type, grade, quantity, balance, source, note, created_at as createdAt FROM inventory_transactions ORDER BY created_at DESC LIMIT 100';
  const rows = styleNo ? queryWithParams(sql, [styleNo]) : query(sql);
  res.json(rows.map(r => ({ ...r, grade: r.grade || 'A' })));
}));
