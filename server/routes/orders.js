...
export const setupOrderRoutes = (queryWithParams, query, run, runNoSave, withTx, asyncHandler) => {
  ...
  const listOrders = () => {
    ...
    return query(`SELECT id, date, client, ${fields} FROM orders ORDER BY date DESC`).map(r => ({ ...r, isLargeOrder: !!r.isLargeOrder, largeOrderAck: !!r.largeOrderAck, warehouseAllocation: r.warehouseAllocation ? JSON.parse(r.warehouseAllocation) : null }));
  };
