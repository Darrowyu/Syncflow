        ...
        <Modal isOpen={isOpen} onClose={onClose} title={t('edit_order')} titleIcon={<Edit2 size={20} />}>
            <div className="space-y-4 max-h-[60vh] overflow-y-auto">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div><label className="block text-xs font-medium text-slate-500 mb-1">{t('field_ship_date')}</label><input type="date" className="w-full border border-slate-300 rounded-lg p-2 text-sm" value={order.expectedShipDate || ''} onChange={(e) => onChange({ ...order, expectedShipDate: e.target.value })} /></div>
