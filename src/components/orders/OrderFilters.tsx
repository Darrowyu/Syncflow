    ...
    <div className="flex justify-end items-center flex-wrap gap-2">
      <div className="flex flex-wrap gap-2">
        <div className="bg-slate-100 dark:bg-slate-800 rounded-lg p-1 flex">
          <button onClick={() => onViewModeChange('table')} className={`px-3 py-1.5 rounded text-sm font-medium transition ${viewMode === 'table' ? 'bg-white dark:bg-slate-700 shadow text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400'}`}>{t('view_table')}</button>
          <button onClick={() => onViewModeChange('calendar')} className={`px-3 py-1.5 rounded text-sm font-medium transition flex items-center ${viewMode === 'calendar' ? 'bg-white dark:bg-slate-700 shadow text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400'}`}><Calendar size={14} className="mr-1" />{t('view_calendar')}</button>
        </div>
        {viewMode === 'table' && ( // 日历模式下隐藏状态筛选
          <div className="bg-slate-100 dark:bg-slate-800 rounded-lg p-1 flex">
            <button onClick={() => onTabChange('all')} className={`px-3 py-1.5 rounded text-sm font-medium transition flex items-center ${activeTab === 'all' ? 'bg-white dark:bg-slate-700 shadow text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400'}`}><Package size={14} className="mr-1" />{t('tab_pending')} <span className="ml-1 text-xs bg-slate-200 dark:bg-slate-600 px-1.5 rounded">{allCount}</span></button>
        ...
        <button onClick={onAiImport} className="flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition"><Bot size={18} className="mr-2" />{t('btn_import_ai')}</button>
      </div>
    </div>
  );
});
