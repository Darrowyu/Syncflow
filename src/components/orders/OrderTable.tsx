...
const OrderTable: React.FC<OrderTableProps> = memo(({ orders, inventory, lines, expandedId, onToggleExpand, onUpdateStatus, onAcknowledgeOrder, onEdit, onDelete, onPrint }) => {
  const { t } = useLanguage();
  ...
  const fulfillmentMap = useMemo(() => {
    ...
    orders.forEach(order => map.set(order.id, calculateFulfillment(order, inventory, lines)));
    return map;
  }, [orders, inventory, lines]);
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
      ...
      <div className="bg-slate-100 dark:bg-slate-900 border-t-2 border-slate-300 dark:border-slate-600 px-3 py-3 flex justify-between text-sm">
        <span className="text-slate-500 dark:text-slate-400 font-medium">{t('total_summary')} ({orders.length} {t('order_unit')})</span>
        <div className="flex space-x-6">
          <span className="font-mono font-bold text-slate-800 dark:text-slate-100">{totals.tons.toFixed(2)}t</span>
          <span className="font-mono font-medium text-slate-700 dark:text-slate-300">{totals.containers} {t('container_unit')}</span>
        </div>
      </div>
    </div>
  );
